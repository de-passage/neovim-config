...
local nmap = require 'utility'.nmap

local function load_dap()
  require 'config.telescope'.setup()

  for _, v in pairs({ 'nvim-dap', 'nvim-dap-ui', 'nvim-dap-virtual-text' }) do
    vim.cmd("packadd " .. v)
  end
  vim.api.nvim_del_user_command('LoadDap')
  ...
  if vim.loop.fs_stat(filename) then
    ...
    if ok then
      print("Local DAP configuration loaded")
    else
...
vim.api.nvim_create_user_command('LoadDap', load_dap, { nargs = 0, desc = 'Load and set up the DAP packages' })
